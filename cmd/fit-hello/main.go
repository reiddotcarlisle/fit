...
func getInt(args []string, name string) (int, bool) {
    for i := 0; i < len(args)-1; i++ {
        if args[i] == name {
            v, err := strconv.Atoi(args[i+1]); if err == nil { return v, true }
            return 0, false
        }
    }
    return 0, false
...
func writeJSON(v any) {
    ...
    if err != nil { log.Fatalf("json: %v", err) }
    os.Stdout.Write(b)
...
func getHelloWindow() (*hiddenwindow.HiddenWindow, func()) {
    ...
    if err != nil { log.Fatalf("hidden window: %v", err) }
...
func cmdList(args []string) {
    ...
    if err != nil { log.Fatalf("PlatformCredentialList: %v", err) }
    if has(args, "--json") {
    ...
    if len(creds) == 0 { if rp != "" { fmt.Printf("No credentials for RP '%s'\n", rp) } else { fmt.Println("No credentials found.") }; return }
...
func cmdTest(args []string) {
    ...
    if rp != "" { fmt.Printf("  RP filter: %s\n", rp) }
    fmt.Printf("  Credentials found: %d\n", len(creds))
    max := len(creds); if max > 5 { max = 5 }
    for i := 0; i < max; i++ {
        c := creds[i]
        fmt.Printf("    [%d] RP=%s User=%s Removable=%v BackedUp=%v CredID(b64url)=%s\n", i, c.RP.ID, c.User.Name, c.Removable, c.BackedUp, base64.RawURLEncoding.EncodeToString(c.CredentialID))
    }
    if len(creds) > max { fmt.Printf("    ... and %d more\n", len(creds)-max) }
}

func cmdAuth(args []string) {
    rp := getString(args, "--rp")
    if rp == "" { fmt.Println("Usage: auth --rp RP [--cred-id-hex HEX|--cred-id-b64 B64URL|--cred-index N|--device]"); return }

    wnd, closeWnd := getHelloWindow(); defer closeWnd()

    // determine credential
    var credID []byte
    if hexStr := getString(args, "--cred-id-hex"); hexStr != "" {
        b, err := hex.DecodeString(strings.TrimSpace(hexStr)); if err != nil { log.Fatalf("--cred-id-hex: %v", err) }
        credID = b
    } else if b64 := getString(args, "--cred-id-b64"); b64 != "" {
        b, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(b64)); if err != nil { log.Fatalf("--cred-id-b64: %v", err) }
        credID = b
    } else if idx, ok := getInt(args, "--cred-index"); ok {
        list, err := winhello.PlatformCredentialList(rp, false); if err != nil { log.Fatalf("PlatformCredentialList: %v", err) }
        if idx < 0 || idx >= len(list) { log.Fatalf("--cred-index out of range (have %d)", len(list)) }
        credID = list[idx].CredentialID
    } else if has(args, "--device") {
        // leave credID nil to open OS selector for external keys
    } else {
        list, err := winhello.PlatformCredentialList(rp, false); if err != nil { log.Fatalf("PlatformCredentialList: %v", err) }
        if len(list) == 0 { fmt.Println("No Windows Hello credentials found for RP."); return }
    ...
    if err != nil { log.Fatalf("GetAssertion: %v", err) }
...
func cmdAddPasskey(args []string) {
    rp := getString(args, "--rp")
    if rp == "" { fmt.Println("Usage: add-passkey --rp RP [--user USER] [--display NAME] [--device] [--resident|--no-resident]"); return }
    user := getString(args, "--user"); if user == "" { user = "fit-user" }
    display := getString(args, "--display"); if display == "" { display = user }
    resident := true; if has(args, "--no-resident") { resident = false }
    ...
    if err != nil { log.Fatalf("MakeCredential: %v", err) }
...
func cmdDeletePasskey(args []string) {
    ...
    if hexStr != "" {
        ...
        b, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(b64Str)); if err != nil { log.Fatalf("--cred-id-b64: %v", err) }
        credID = b
    } else if hasIdx {
        list, err := winhello.PlatformCredentialList(rp, false); if err != nil { log.Fatalf("PlatformCredentialList: %v", err) }
        if idx < 0 || idx >= len(list) { log.Fatalf("--cred-index out of range (have %d)", len(list)) }
    ...
    if err := winhello.DeletePlatformCredential(credID); err != nil { log.Fatalf("DeletePlatformCredential: %v", err) }
    if has(args, "--json") { writeJSON(map[string]any{"backend":"hello","deleted":true,"credID":base64.RawURLEncoding.EncodeToString(credID)}) } else { fmt.Println("Credential deleted.") }
}
